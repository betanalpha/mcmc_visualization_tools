...
putil.plot_conn_pushforward_quantiles(
  plot.gca(), samples, pred_names,
  data['x_grid'], xlabel="x", ylabel="f"
)
plot.show()
```
...
```{python}
pred_names = [ f'y_pred_grid[{n + 1}]'
               for n in range(data['N_grid']) ]
putil.plot_conn_pushforward_quantiles(
  plot.gca(), samples, pred_names,
  data['x_grid'], xlabel="x", ylabel="f"
)
...
putil.plot_realizations(axarr[0], samples,
                        ...
                        xlabel="x", ylabel="f")
putil.plot_conn_pushforward_quantiles(
  axarr[1], samples, f_names, data['x_grid'],
  baseline_values=true_fs, baseline_color=putil.mid_teal,
  xlabel="x", ylabel="f"
)
...
putil.plot_conn_pushforward_quantiles(
  axarr[1], samples, f_names, data['x_grid'],
  residual=True, baseline_values=true_fs,
  xlabel="x", ylabel="f"
)
...
putil.plot_line_hist(axarr[0], data['X'][:,0],
                     -9, 9, 1, xlabel="x1")
putil.plot_line_hist(axarr[1], data['X'][:,1],
                     -9, 9, 1, xlabel="x2")
putil.plot_line_hist(axarr[2], data['X'][:,2],
                     -9, 9, 1, xlabel="x3")
...
axarr[0, 0].scatter(data['X'][:,0], data['X'][:,1],
                    color="black", s=2)
...
axarr[0, 1].scatter(data['X'][:,0], data['X'][:,2],
                    color="black", s=2)
...
axarr[0, 2].scatter(data['X'][:,1], data['X'][:,2],
                    color="black", s=2)
...
axarr[1, 0].scatter(data['X'][:,0], data['y'],
                    color="black", s=2)
...
axarr[1, 1].scatter(data['X'][:,1], data['y'],
                    color="black", s=2)
...
axarr[1, 2].scatter(data['X'][:,2], data['y'],
                    color="black", s=2)
...
pred_names = [ f'y_pred[{n + 1}]'
               for n in range(data['N']) ]
putil.plot_conditional_mean_quantiles(
  axarr[0], samples, pred_names,
  data['X'][:,0], -9, 9, 1,
  xlabel="x1", ylabel=""
)
putil.plot_conditional_mean_quantiles(
  axarr[1], samples, pred_names,
  data['X'][:,1], -9, 9, 1,
  xlabel="x2", ylabel=""
)
putil.plot_conditional_mean_quantiles(
  axarr[2], samples, pred_names,
  data['X'][:,2], -9, 9, 1,
  xlabel="x3", ylabel=""
)
plot.show()
...
names = [ f'beta1[{m + 1}]'
          for m in range(data['M']) ]
putil.plot_disc_pushforward_quantiles(
  axarr[0], samples, names,
  xlabel="beta1",
  xticklabels=[ m + 1 for m in range(data['M']) ],
...
This plot style can also be rotated, which is helpful when working
with longer labels.

```{python}
f, axarr = plot.subplots(3, 1, layout="constrained")
